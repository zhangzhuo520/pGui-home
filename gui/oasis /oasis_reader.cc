...
OasisReader::OasisReader() : m_byte_buffer(1024), m_file_buffer(FILE_BUF_SIZE), m_cblock_buffer(FILE_BUF_SIZE), m_in_cblock(false)
{
    ...
    if(m_decompressor->initialize() < 0)
    {
        throw OasisException("decompressor initialize failed");
    }
    m_file_offset = 0;
...
void OasisReader::feed_cblock_buffer()
{
    ...
    {
        //here we must NOT use bufferEmpty(), the data pointer point to m_cblockBuf
        if(m_file_buffer.empty())
        {
            feed_file_buffer();
        }

        //here feed all compressed data in once
    ...
    uint32 fetchedSize;
    if(m_decompressor->fetch_data(m_cblock_buffer.begin, FILE_BUF_SIZE, fetchedSize) < 0)
    {
        throw OasisException("fetch decompressed data failed");
    }
    if(fetchedSize == 0)
    {
        if(m_uncompressed_bytes_left > 0 && m_compressed_bytes_left > 0)
...
void OasisReader::enter_cblock(uint64 uncomp_byte_count, uint64 comp_byte_count)
{
    ...
    if(m_decompressor->prepare_decompress() != 0)
    {
        oas_assert(false);
    }
...
const OasisString OasisReader::read_string()
{
    OasisString ostring;
    read_string(ostring);
    return ostring;
...
const OasisReal OasisReader::read_real()
{
    OasisReal real;
    read_real(real);
    return real;
...
const OasisDelta OasisReader::read_delta_1(bool horizontal)
{
    OasisDelta delta;
    read_delta_1(delta, horizontal);
    return delta;
}
...
const OasisDelta OasisReader::read_delta_2()
{
    OasisDelta delta;
    read_delta_2(delta);
    return delta;
...
const OasisDelta OasisReader::read_delta_3()
{
    OasisDelta delta;
    read_delta_3(delta);
    return delta;
...
const OasisDelta OasisReader::read_delta_g()
{
    OasisDelta delta;
    read_delta_g(delta);
    return delta;
...
const OasisRepetition OasisReader::read_repetition()
{
    OasisRepetition repetition;
    read_repetition(repetition);
    return repetition;
}
...
void OasisReader::read_repetition(OasisRepetition& repetition)
{
    ...
    switch(repetition.m_type)
    {
        ...
        case OasisRepetition::ORT_DiagonalRegular2D:
            ...
            repetition.m_deltas[0] = read_delta_g();
            repetition.m_deltas[1] = read_delta_g();
            break;
        ...
        case OasisRepetition::ORT_Arbitory:
            ...
            {
                repetition.m_deltas[i] = read_delta_g();
            }
            break;
...
const OasisPointList OasisReader::read_point_list()
{
    OasisPointList pointlist;
    read_point_list(pointlist);
    return pointlist;
}
...
const OasisPropValue OasisReader::read_prop_value()
{
    OasisPropValue propvalue;
    read_prop_value(propvalue);
    return propvalue;
}
...
const OasisInterval OasisReader::read_interval()
{
    OasisInterval interval;
