...
{
    template<typename V>
    static int64 rounded(V v) { return std::numeric_limits<V>::is_integer ? (int64)v : (int64)rint(v); }
};
...
public:
    ...
    TemplatePoint(const TemplatePoint& rh) : m_x(rh.x()), m_y(rh.y()) {}
...
public:
    ...
    TemplateBox(const TemplateBox& rh) : m_left(rh.left()), m_bottom(rh.bottom()), m_right(rh.right()), m_top(rh.top()) {}
    ...
    {
        return TemplateBox<C>(std::numeric_limits<C>::lowest(), std::numeric_limits<C>::lowest(), std::numeric_limits<C>::max(), std::numeric_limits<C>::max());
    }
private:
    C m_left;
    C m_bottom;
...
#define OAS_ERR     OasisDebug::loglevel() >= 0 && logger::error
