...
{
public:
    OasisCell(bool import = false) :
        m_stick(false),
        m_saved(false),
        m_cached(true),
        m_deep_info_ready(false),
        m_geom_deep_info_ready(false),
        m_children_ready(false),
        m_import_mode(import),
        m_locker(true),
        m_use_count(0),
        m_use_time(0)
    {
    ...
    void compute_deep_information(OasisLayout& layout);
    bool try_compute_deep_information(OasisLayout& layout);
    void set_layers(const std::set<std::pair<int32, int32> >& layer_set) { m_layer_set = layer_set;}
    void combine_layers(std::set<std::pair<int32, int32> >& parent_layer_set) const;
    bool has_layer(int32 layer, int32 datatype) const { return m_layer_set.count(std::pair<int32, int32>(layer, datatype)) != 0; }
    ...
    const Box& get_bbox() const { oas_assert(m_deep_info_ready); return m_bbox; }
    void set_bbox(const Box& bbox) { m_bbox = bbox; m_deep_info_ready = true; }
...
private:
    ...
    std::set<std::pair<int32, int32> > m_layer_set;
    ...
    bool m_saved;
    bool m_cached;
    bool m_deep_info_ready;
    bool m_geom_deep_info_ready;
