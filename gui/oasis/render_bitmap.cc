...
uint32_t* Bitmap::scanline(unsigned int n)
{
    ...
    if(row_data == 0)
    {
        ...
        // clear the data in the pointer orginated from the free scanlines
...
void Bitmap::init(unsigned int width,unsigned int height)
{
    ...
    m_first_line = std::numeric_limits<unsigned int>::max();
    m_last_line = 0;
}
...
void Bitmap::merge(Bitmap* copyee, int x, int y)
{
    if(!copyee)
    {
        return;
    ...
    unsigned int copyee_height = copyee->height();
    if(int(copyee_height) + y > int(height()))
    {
        copyee_height =  height() - y;
    }

    unsigned int copyee_width = copyee->width();
    if(int(copyee_width + x) > int(width()))
    {
        copyee_width = width() - x;
    }

    unsigned int first_line  = 0;
    if(y < 0)
    {
        if(y + (int)copyee_height <= 0)
        {
            return;
        }
        first_line = (unsigned int) -y;
    }

    if(x < 0)
    {
        if(x + int(copyee_width) <= 0)
        {
            return;
        }

        unsigned int current_seg = ((unsigned int) -x) /32;
        unsigned int copyee_total_segs = (copyee_width + 31) /32 - current_seg;
        unsigned int total_segs_plus = (copyee_width + x + 31) /32;
        unsigned int s1 = ((unsigned int) - x) % 32;
        unsigned int s2 = 32 - s1;


        for(unsigned int i = first_line; i < copyee_height; ++i)
        {
            if(copyee->is_empty_scanline(i))
            {
                continue;
            }
            const uint32_t* scanline_from = copyee->scanline(i) + current_seg;
            uint32_t* scanline_to =  scanline(i + y);

            if(!s1)
            {
                for(unsigned int j = 0; j < copyee_total_segs; j++)
                {
            ...
            {
                for(unsigned int j = 1; j < copyee_total_segs; j++)
                {
                    *scanline_to++ |= (scanline_from[1] << s2) | (scanline_from[0] >> s1);
                    scanline_from ++;
                }
                if(total_segs_plus > copyee_total_segs - 1)
                {
                    *scanline_to ++ |= (scanline_from[0] >> s1);
                }
            }
         }
        ...
        unsigned int current_seg = ((unsigned int) x) / 32;
        unsigned int copyee_total_segs = (copyee_width + 31) /32;
        unsigned int total_segs_plus = (copyee_width + (((unsigned int) x) % 32) + 31) /32;
        unsigned int s1 = ((unsigned int) x ) % 32;
        unsigned int s2 = 32 - s1;
        ...
        for(unsigned int i = first_line; i< copyee_height; ++i)
        {
            ...
            const uint32_t* scanline_from = copyee->scanline(i);
            uint32_t* scanline_to = scanline(i + y) + current_seg;
            ...
            else if(copyee_total_segs)
            {
                *scanline_to++ |= scanline_from[0] << s1;
                for(unsigned int j = 1; j < copyee_total_segs; j++)
                {
                    *scanline_to++ |= (scanline_from[0] >> s2) | (scanline_from[1] << s1);
                    ++scanline_from;
                }
                if(total_segs_plus > copyee_total_segs)
                {
                    *scanline_to++ |= (scanline_from[0] >> s2);
                }
            }
        }
    }
