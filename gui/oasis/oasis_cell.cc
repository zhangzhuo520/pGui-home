...
void OasisProperty::write(OasisWriter& writer)
{
    ...
    if(!same_name)
    {
        if(m_prop_name.is_referenced())
        {
            writer.write_uint(m_prop_name.get_index());
        }
        else
        {
    ...
    bool on_right)
{
    ...
    float64 dx2 = C.x() - B.x();
    float64 dy2 = C.y() - B.y();
    float64 r1 = std::sqrt(dx1 * dx1 + dy1 * dy1);
    float64 r2 = std::sqrt(dx2 * dx2 + dy2 * dy2);
    float64 dr1 = halfwidth / r1;
    float64 dr2 = halfwidth / r2;
    float64 xs = on_right ? 1 : -1;
    float64 ys = on_right ? -1 : 1;
    float64 x1 = A.x() + xs * dr1 * dy1;
    float64 y1 = A.y() + ys * dr1 * dx1;
    float64 x2 = B.x() + xs * dr2 * dy2;
    float64 y2 = B.y() + ys * dr2 * dx2;
    float64 l = (( y2 - y1 ) * dx1 - ( x2 - x1 ) * dy1) / (dx2 * dy1 - dy2 * dx1);

    poly_new.set_x((int64)rint( x2 + l * dx2 ));
    poly_new.set_y((int64)rint( y2 + l * dy2 ));

    if (abs(B.x() - A.x()) == abs(B.y() - A.y()))
    {
        if(abs(poly_new.x() - poly_pre.x()) != abs(poly_new.y() - poly_pre.y()))
        {
            int64 best_x = 0;
            int64 best_y = 0;
            float64 best_d = 1e5;
            bool fixed = false;
            for (int32 step_x = -1; step_x <= 1; ++step_x)
            {
                for (int32 step_y = -1; step_y <= 1; ++step_y)
                {
                    int64 try_x = poly_new.x() + step_x;
                    int64 try_y = poly_new.y() + step_y;
                    if (abs(try_x - poly_pre.x()) == abs(try_y - poly_pre.y()))
                    {
                        float64 d = fabs(((try_y - y2) * dx2) - ((try_x - x2) * dy2));
            ...
            if (fixed)
            {
                poly_new.set_x(best_x);
                poly_new.set_y(best_y);
                return;
            }
        }
    }
...
static void path_to_polygon(std::vector<Point>& path,
                                std::vector<Point>& poly,
                                uint64 halfwidth, int64 startext, int64 endext)
{
    uint32 right_index = 0;
    uint32 left_index = 2 * path.size() - 1;
    poly.resize(path.size() * 2);

    if (path[0].x() == path[1].x())
    {
        int64 sign = path[0].y() < path[1].y() ? 1 : -1;
        poly[right_index].set_x(path[0].x() + sign * halfwidth);
        poly[left_index].set_x(path[0].x() - sign * halfwidth);
        poly[right_index].set_y(path[0].y() - sign * startext);
        poly[left_index].set_y(path[0].y() - sign * startext);
    }
    else if (path[0].y() == path[1].y())
    {
        int64 sign = path[0].x() < path[1].x() ? 1 : -1;
        poly[right_index].set_y(path[0].y() - sign * halfwidth);
        poly[left_index].set_y(path[0].y() + sign * halfwidth);
        poly[right_index].set_x(path[0].x() - sign * startext);
        poly[left_index].set_x(path[0].x() - sign * startext);
    }
    else
    {
        ...
        poly[right_index].set_x(path[0].x() - ddx);
        poly[right_index].set_y(path[0].y() - ddy);
        poly[left_index].set_x(path[0].x() - ddy);
        poly[left_index].set_y(path[0].y() + ddx);
    }
    ...
    for (; i < path.size() - 1; ++i, ++right_index, --left_index)
    {
        if (path[i].x() == path[i-1].x())
        {
            if (path[i].y() == path[i+1].y())
            {
                int64 signx = path[i].y() > path[i - 1].y() ? 1 : -1;
                poly[right_index].set_x(path[i].x() + signx * halfwidth);
                poly[left_index].set_x(path[i].x() - signx * halfwidth);

                int64 signy = path[i].x() < path[i + 1].x() ? 1 : -1;
                poly[right_index].set_y(path[i].y() - signy * halfwidth);
                poly[left_index].set_y(path[i].y() + signy * halfwidth);
            }
            else
            {
                intersect_path( path[i - 1], path[i], path[i + 1], halfwidth, poly[right_index - 1], poly[right_index], true );
                intersect_path( path[i - 1], path[i], path[i + 1], halfwidth, poly[left_index + 1], poly[left_index], false );
            }
        }
    ...
    if( path[i].x() == path[i-1].x() )
    {
        int64 sign = path[i].y() < path[i-1].y() ? 1 : -1;
        poly[right_index].set_x(path[i].x() - sign * halfwidth);
        poly[left_index].set_x(path[i].x() + sign * halfwidth);
        poly[right_index].set_y(path[i].y() - sign * endext);
        poly[left_index].set_y(path[i].y() - sign * endext);
    }
    else if( path[i].y() == path[i-1].y() )
    {
        int64 sign = path[i].x() < path[i - 1].x() ? 1 : -1;
        poly[right_index].set_y(path[i].y() + sign * halfwidth);
        poly[left_index].set_y(path[i].y() - sign * halfwidth);
        poly[right_index].set_x(path[i].x() - sign * endext);
        poly[left_index].set_x(path[i].x() - sign * endext);
    }
    else
    {
        ...
        poly[right_index].set_x(path[i].x() + ddx);
        poly[right_index].set_y(path[i].y() + ddy);
        poly[left_index].set_x(path[i].x() - ddy);
        poly[left_index].set_y(path[i].y() + ddx);
    }
}
...
void PathElement::polygon(Polygon& p)
{
    ...
    {
        OAS_WARN << "invalid path" << OAS_ENDL;
        return;
    }
    ...
    path_to_polygon(path_points, polygon_points, m_halfwidth, m_start_ext, m_end_ext);
...
void TextElement::write(OasisWriter& writer)
{
    ...
    if(info_byte & HasExplicitText)
    {
        ...
        writer.write_uint(m_textstring.get_index());
    }
...
void PlacementElement::write(OasisWriter& writer)
{
    ...
    else
    {
        ...
        info_byte |= ((m_trans.rotation() << 1) & AngleMask);
        writer.write_byte(info_byte);
    }
...
void OasisCell::compute_deep_information(OasisLayout& layout)
{
    sys::MutexGuard guard(m_locker);
    if(m_deep_info_ready)
    {
        return;
    }

    OAS_DBG << "compute deep information " << get_infoindex() << OAS_ENDL;
    ...
    for(uint32 i = 0; i < m_geometries.size(); ++i)
    {
        ...
        m_bbox.update(box);
        m_layer_set.insert(std::pair<int32, int32>(e->layer(), e->datatype()));
    }
    for(uint32 i = 0; i < m_texts.size(); ++i)
    {
        ...
        m_bbox.update(box);
        m_layer_set.insert(std::pair<int32, int32>(t->layer(), t->datatype()));
    }

    std::set<int64> dup_instances;
    for(uint32 i = 0; i < m_instances.size(); ++i)
    {
        ...
        OasisCell& cell = layout.get_cell(info.get_cell_index());
        cell.compute_deep_information(layout);

        Box box = p->get_bbox_with_repetition(layout);
        m_bbox.update(box);

        if (!dup_instances.count(p->get_info_index()))
        {
            dup_instances.insert(p->get_info_index());
            cell.combine_layers(m_layer_set);
        }
    }
    ...
    m_deep_info_ready = true;
    OAS_DBG << "deep information ready " << get_infoindex() << OAS_ENDL;
}

bool OasisCell::try_compute_deep_information(OasisLayout& layout)
{
    sys::MutexGuard guard(m_locker);
    if(m_deep_info_ready)
    {
    ...
    OAS_DBG << "try compute deep information " << get_infoindex() << OAS_ENDL;
    ...
    for(uint32 i = 0; i < m_instances.size(); ++i)
    {
        ...
        OasisCell& cell = layout.get_cell(info.get_cell_index());
        if(!cell.try_compute_deep_information(layout))
        {
            decrease_use_count();
            return false;
        }
...
void OasisCell::combine_layers(std::set<std::pair<int32, int32> >& parent_layer_set) const
{
    for (auto lit = m_layer_set.begin(); lit != m_layer_set.end(); ++lit)
    {
        parent_layer_set.insert(*lit);
    }
}
...
void OasisCell::assign_raw_data(OasisLayout &layout, const Box &bbox, const std::vector<std::pair<int32, int32> > &layers, const uint8 *data, int32 data_size)
{
    ...
    m_hint_datasize = writer.get_offset() - m_fileoffset + 1;
    m_saved = true;
...
void OasisCell::partition_instance(OasisLayout& layout)
{
    ...
    if(!uncomplete_instances.empty())
    {
        ...
        for(uint32 i = 0; i < grid_instances.size(); ++i)
        {
            ...
            cell.try_compute_deep_information(layout);
            cell.unload(layout);
        }
    }
...
void OasisCell::partition_geometry(OasisLayout& layout)
{
    ...
    compute_deep_information(layout);
...
void OasisCell::flush(OasisLayout& layout)
{
    if(!m_saved)
    {
        ...
        else
        {
            ...
            m_hint_datasize = memory_writer.get_size() + 1;
            m_saved = true;
            OAS_DBG << "flush " << get_infoindex() << " offset " << m_fileoffset << OAS_ENDL;
        }
    }
...
void OasisCell::load(OasisLayout& layout)
{
    if (m_import_mode)
    {
        sys::MutexGuard guard(m_locker);
        import_load(layout);
        layout.change_imported_element_count(cached_element_count());
    }
    else
    {
        sys::MutexGuard guard(m_locker);
...
void OasisCell::parse_load(OasisLayout& layout)
{
    ...
    layout.writer().get_locker().Lock();
    layout.flush_writer(layout.writer());
    layout.writer().get_locker().Unlock();
...
uint64 OasisCell::cached_element_count()
{
    if(!m_cached)
    {
        return 0;
...
void OasisCell::clone(OasisCell& dest, OasisLayout& layout, int64 infoindex)
{
    ...
    dest.m_cached = true;
    dest.m_bbox = m_bbox;
    dest.m_deep_info_ready = m_deep_info_ready;
    dest.m_geom_deep_info_ready = m_geom_deep_info_ready;
}
...
void OasisCell::unload(OasisLayout& layout)
{
    if (m_import_mode)
    {
        ...
        if (unused())
        {
            layout.change_imported_element_count(-(int64)cached_element_count());
            cleanup();
        }
    }
    else
...
void OasisCellInfo::get_properties(OasisLayout& layout, std::vector<OasisProperty>& properties)
{
    ...
    cell.combine_layers(cellprop.m_layers);
