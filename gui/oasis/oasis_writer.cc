...
void OasisWriter::leave_cblock()
{
    ...
    std::vector<uint8> outbuf(m_cblock_buffer.used());

    m_compressor->prepare_compress();
    ...
    do
    {
        m_compressor->fetch_data(&outbuf[0], outbuf.size(), fetchedSize);
        m_compressed_buffer.write(&outbuf[0], fetchedSize);
    } while (fetchedSize > 0);
