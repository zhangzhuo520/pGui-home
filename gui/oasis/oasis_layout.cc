...
uint64 OasisLayout::imported_element_count()
{
    sys::MutexGuard guard(m_element_count_locker);
...
int32 OasisLayout::count_mark(int64 top_index, 
                                 const std::string& subname, 
                                 std::map<std::string, std::string> &parent_to_subcell, 
                                 std::map<std::string, int> &corner_to_count,
                                 std::map<std::string, std::vector<oasis::OasisTrans>> &instances_map)
{
    if (top_index < 0)
    {
        int32 count = 0;
        for(uint32 i = 0; i < m_topcellinfos.size(); ++i)
        {
            count += count_mark(m_topcellinfos[i], subname, parent_to_subcell, corner_to_count, instances_map);
        }
        return count;
    }
    ...
    OasisCellInfo &cellinfo = get_cell_info(top_index);
    OasisCell& cell = get_cell(cellinfo.get_cell_index());
    CountMarkVisitor visitor(subname, parent_to_subcell, corner_to_count, instances_map);
    visitor.set_layout(this);
    visitor.set_top_query_box(Box::world());
    visitor.set_top_trans(OasisTrans());
...
void OasisLayout::draw_bitmap(int64 infoindex,
                              unsigned long levels,
                              int block_index,
                              Box query_box,
                              OasisTrans trans,
                              OasisTrans dbu_trans,
//                              std::map<LDType, int>& ld_map,
                              std::unordered_map<LDType, int, render::hash_LDType, render::cmp_LDType>&ld_map,
...

{
    if(infoindex < 0)
    {
        for(uint32 i = 0; i < m_topcellinfos.size(); i++)
        {
            draw_bitmap(m_topcellinfos[i], levels, block_index, query_box, trans, dbu_trans, ld_map, bitmaps, cache, thread);
        }
        return ;
    }
    update_time();
    OasisCellInfo& cellinfo = get_cell_info(infoindex);
    OasisCell& cell = get_cell(cellinfo.get_cell_index());
    DrawBitmapVisitor visitor(levels, block_index, dbu_trans, ld_map, bitmaps, cache, thread);
    visitor.set_layout(this);
    visitor.set_top_query_box(query_box.intersected(cell.get_bbox()));
    ...
    visitor.print_hits(block_index);
}
...
void OasisLayout::print_layers() const
{
    OAS_INFO << "layers in all cell:" << OAS_ENDL;
    for (uint32 i = 0; i < get_cell_count(); ++i)
    {
        std::set<std::pair<int32, int32> > layers;
        const OasisCellInfo &info = get_cell_info(i);
        const OasisCell &cell = get_cell(info.get_cell_index());
        cell.combine_layers(layers);
        OAS_INFO << "cell index " << i << ", name " << info.get_name() << " layers: " << OAS_ENDL;
        for (auto it = layers.begin(); it != layers.end(); ++it)
        {
            OAS_INFO << "\t" << it->first << "/" << it->second << OAS_ENDL;
        }
    }
}
...
void OasisLayout::compute_deep_information()
{
    OAS_INFO << "begin layout compute deep information" << OAS_ENDL;
    for(uint32 i = 0; i < m_topcellinfos.size(); ++i)
    {
        int64 index = m_topcellinfos[i];
        OasisCellInfo& cellinfo = get_cell_info(index);
        OasisCell& cell = get_cell(cellinfo.get_cell_index());
        cell.compute_deep_information(*this);

        cell.combine_layers(m_layers);
    }
    OAS_INFO << "end layout compute deep information" << OAS_ENDL;
}
...
void OasisLayout::change_imported_element_count(int64 count)
{
    sys::MutexGuard guard(m_element_count_locker);
    m_element_count += count;
    OAS_DBG << "change imported element count " << count << ", total " << m_element_count << OAS_ENDL;
}

void OasisLayout::smart_clean_cache(int64 cache_limit, int64 expire_limit, int64 cycle_limit)
{
    OAS_DBG << "smart clean cache called. "
        << "cache limit " << cache_limit << ","
        << "expire limit " << expire_limit << ","
        << "cycle limit " << cycle_limit << OAS_ENDL;
    ...
    {
        int64 object_count = imported_element_count();
        int64 unload_cycle = 0;
        int64 expire = expire_limit;
        while (object_count > cache_limit && unload_cycle < cycle_limit)
        {
            OAS_DBG << "before clean: "
                << "cycle " << unload_cycle << ","
                << "expire " << expire << ","
                << "element count " << object_count << OAS_ENDL;

            unload(false, expire);

            object_count = imported_element_count();
            expire /= 2;
            unload_cycle++;
