...
void MainWindow::initDockWidget()
{
    static const struct Set {
        ...
        { "Guage", 0, Qt::RightDockWidgetArea }
    };
...
{
    double start_point_x = index.sibling(index.row(), 4).data().toDouble() / 1000;
    double start_point_y = index.sibling(index.row(), 5).data().toDouble() / 1000;

    double end_point_x = index.sibling(index.row(), 6).data().toDouble() / 1000;
    double end_point_y = index.sibling(index.row(), 7).data().toDouble() / 1000;
    QString line_info = index.sibling(index.row(), 0).data().toString();

    //move point
    if ((m_current_tabid < m_paint_tabwidget->count()) && (m_paint_tabwidget->count() > 0))
    {
        m_paint_tabwidget->get_scaleframe(m_current_tabid)->set_defect_point((end_point_x + start_point_x) / 2, (end_point_y + start_point_y) / 2);
    }
    else
    {
        showWarning(this, "Warning", "Not open the canvas!");
...
void MainWindow::close_defgroup_widget(int job_number)
{
    QString defgroup = "Job" + QString::number(job_number) +"_defGroup";
    DockWidget *defgroup_widget = findChild < DockWidget * >(defgroup);
...
void MainWindow::close_defects_widget(int job_number)
{
...
void MainWindow::add_file(QString filePath, bool isOverLay)
{
    ...
    logger_widget("Open file : " + filePath);
    logger_widget("CUP Usage: " + QString::number(Global::CPU_usage()));
